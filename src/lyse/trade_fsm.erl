...
%% Responding to other player's reqeusts to negotiate
...
%% Ask other player's to negotiate
...
%% Moving to idle_wait as result of asking another player to negotiate
idle_wait({ask_negotiate, OtherPid}, S = #state{other = OtherPid}) ->
  gen_fsm:reply(S#state.from, ok),
  notice(S, "starting negotitation", []),
...
%% Moving to idle_wait as a result of another player asking US to negotiate
idle_wait(accept_negotiate, _From, S = #state{other = OtherPid}) ->
  accept_negotiate(OtherPid, self()),
  notice(S, "accepting negotation", []),
...
%% Other Side Making Offer
negotiate({do_offer, Item}, S = #state{otheritems = OtherItems}) ->
  notice(S, "other player is offering ~p", [Item]),
  {next_state, negotiate, S#state{otheritems = add(Item, OtherItems)}};
%% Other Side Retracting Offer
negotiate({undo_offer, Item}, S = #state{otheritems = OtherItems}) ->
  notice(S, "other player is retracting offering ~p", [Item]),
  {next_state, negotiate, S#state{otheritems = remove(Item, OtherItems)}};
negotiate(are_you_ready, S = #state{other = OtherPid}) ->
  io:format("other user ready to rtrade.~n"),
...
commit(S = #state{}) ->
  ...
  "This operation should have soem atomic save"
